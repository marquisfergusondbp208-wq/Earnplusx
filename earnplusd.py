...
def _pair_hourly_bg(user_id: int, account: str,
    ...
    if current_account and should_switch_account(current_account["username"], 110):
        log.info(f"[HourlyPair] Account {current_account['username']} has >= 110 numbers. Switching...")
        ...
        if next_account and next_account["username"] != current_account["username"]:
            ...
            if platform_login():
                log.info(f"[HourlyPair] Successfully switched to {next_account['username']}")
                ...
                asyncio.run_coroutine_threadsafe(
                    send_telegram(
                        ...
                        f"Account `{current_account['username']}` reached 110+ numbers.\n"
                        f"Switched to `{next_account['username']}`\n"
                        f"📊 Current numbers: {next_account['hourly_numbers_count']}",
                        parse_mode="Markdown"
                    ),
                    _bot_loop
                )
            else:
                log.error(f"[HourlyPair] Failed to login with {next_account['username']}")
                asyncio.run_coroutine_threadsafe(
                    send_telegram(
                        ADMIN_TELEGRAM_ID,
                        f"⚠️ *Account Switch Failed*\n\n"
                        f"Failed to login with `{next_account['username']}`\n"
                        f"Falling back to `{current_account['username']}`",
                        parse_mode="Markdown"
                    ),
                    _bot_loop
                )
        else:
            log.warning(f"[HourlyPair] No alternative account found! Using current account.")
            asyncio.run_coroutine_threadsafe(
                send_telegram(
                    ADMIN_TELEGRAM_ID,
                    f"⚠️ *No Available Accounts*\n\n"
                    f"Account `{current_account['username']}` has 110+ numbers\n"
                    f"but no other accounts are configured.\n"
                    f"Please add more accounts using /add_account",
                    parse_mode="Markdown"
                ),
                _bot_loop
            )
    ...
    for variant in variants:
        with get_db() as db:
            ...
            if not existing:
                if is_postgres:
                    db.execute(
                        "INSERT INTO numbers(user_id, account, status, hourly_status) VALUES(%s, %s, 'pairing', 'pending')",
                        (user_id, variant)
                    )
                else:
                    db.execute(
    ...
    for attempt_idx, variant in enumerate(variants):
        ...
        log.info(f"[HourlyPair] Attempt {attempt_idx+1}/{max_attempts}: {variant}")
        
        # Update DB for this variant
        with get_db() as db:
            is_postgres = DATABASE_URL is not None
            if is_postgres:
                db.execute(
        ...
        if attempt_idx == max_attempts - 1:
            # Delete all variants that were created but never used
            for v in variants:
                if v != variant:
                    with get_db() as db:
                        is_postgres = DATABASE_URL is not None
    ...
    if not pair_code:
        ...
        asyncio.run_coroutine_threadsafe(
            edit_telegram_message(
                ...
                f"Tried {max_attempts} format(s):\n{tried_str}\n\n"
                "Please check the number and try again from *My Numbers*.\n\n"
                "💡 Tip: Make sure the number is correct and try adding a space between country code and local number.",
                parse_mode="Markdown"
            ),
            _bot_loop
        )
        # DELETE all variant rows
        with get_db() as db:
            is_postgres = DATABASE_URL is not None
            for v in variants:
                if is_postgres:
    ...
    with get_db() as db:
        ...
        if is_postgres:
            db.execute(
                "UPDATE numbers SET pair_code=%s WHERE user_id=%s AND account=%s",
                (pair_code, user_id, used_account)
            )
        else:
            db.execute(
                "UPDATE numbers SET pair_code=? WHERE user_id=? AND account=?",
                (pair_code, user_id, used_account)
            )
    ...
    # If more variants are available, show the "Link Next" button
    next_variant = None
    if variants_enabled and len(variants) > 1:
        ...
        if current_idx < len(variants) - 1:
            next_variant = variants[current_idx + 1]
    ...
    if next_variant:
        reply_markup = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                f"🔄 Link Next: {next_variant}",
                callback_data=f"linkagain_{original}__{next_variant}"
            )
        ]])
    elif len(variants) > 1:
        reply_markup = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                f"🔄 Try Again",
                callback_data=f"linkagain_{original}__{used_account}"
            )
        ]])
    ...
    # ============ POLL FOR NUMBER ONLINE ============
    deadline = time.time() + 600
    came_online = False
    poll_count = 0
    while time.time() < deadline:
        poll_count += 1
        # Update status every 10 polls (every ~40 seconds)
        if poll_count % 10 == 0:
            asyncio.run_coroutine_threadsafe(
                edit_telegram_message(
                    initial_msg,
    ...
    if not came_online:
        ...
        with get_db() as db:
            ...
            # Also delete any unused variants
            for v in variants:
                if v != used_account:
                    if is_postgres:
                        db.execute("DELETE FROM numbers WHERE user_id=%s AND account=%s", (user_id, v))
    ...
    with get_db() as db:
        ...
        # Delete any duplicate variants that might exist
        for v in variants:
            if v != used_account:
                if is_postgres:
                    db.execute("DELETE FROM numbers WHERE user_id=%s AND account=%s", (user_id, v))
...
async def send_telegram(telegram_id: int, text: str, reply_markup=None, parse_mode="HTML"):
    ...
    try:
        ...
        # Store message reference for potential editing
        _message_cache[telegram_id] = msg
...
async def edit_telegram_message(msg, text: str, reply_markup=None, parse_mode="HTML"):
    """Edit an existing telegram message."""
    if msg is None:
        log.error("edit_telegram_message called with None message")
        return
    try:
        await msg.edit_text(
            text=text,
        ...
        log.info(f"✅ Message edited: {text[:50]}...")
    except Exception as e:
