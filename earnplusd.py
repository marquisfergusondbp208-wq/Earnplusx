...
def toggle_hourly_variants(enabled: bool):
    """Enable or disable hourly variants (4-attempt linking)"""
    set_setting("hourly_variants_enabled", "1" if enabled else "0")
    log.info(f"[Hourly] Variants {'enabled' if enabled else 'disabled'}")

def get_hourly_variants_enabled() -> bool:
    """Check if hourly variants are enabled"""
    return get_setting("hourly_variants_enabled", "1") == "1"
...
def pts_to_ngn_fast(points: int) -> float:
    """Convert points to NGN using cached rates - 100x faster (FIXED: better precision)"""
    global _current_npm, _current_ppm, _current_rate_time
    ...
    # ✅ FIXED: Multiply first, then divide to minimize floating point precision loss
    # Was: (points / _current_ppm * _current_npm)
    # Now: (points * _current_npm) / _current_ppm
    return float(points * _current_npm) / float(_current_ppm)
...
def init_db():
    ...
    if DATABASE_URL:
        ...
        try:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS platform_accounts(
                    ...
                    hourly_numbers_count INTEGER DEFAULT 0,
                    created_at TIMESTAMP DEFAULT NOW(),
            ...
            settings_data = [
                ...
                ('hourly_enabled', '1'),
                ('hourly_variants_enabled', '1'),  # <-- NEW SETTING
                ('wsjobs_account', ''),
                ('wsjobs_password', '')
            ]
        ...
        with get_db() as db:
            ...
            INSERT OR IGNORE INTO settings VALUES('hourly_rate_ngn','5.0');
            INSERT OR IGNORE INTO settings VALUES('hourly_monitor_interval_seconds','60');
            INSERT OR IGNORE INTO settings VALUES('hourly_payout_interval_minutes','60');
            INSERT OR IGNORE INTO settings VALUES('hourly_enabled','1');
            INSERT OR IGNORE INTO settings VALUES('hourly_variants_enabled','1');  -- NEW SETTING
            ...
            db.execute("INSERT OR IGNORE INTO settings VALUES('hourly_variants_enabled', '1')")  # NEW
            ...
            # ============ NEW: PLATFORM ACCOUNTS TABLE (SQLITE) ============
            db.execute("""
                CREATE TABLE IF NOT EXISTS platform_accounts(
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    username TEXT NOT NULL UNIQUE,
            ...
            # ✅ FIXED: Add critical indexes for leaderboard & performance
            try:
                db.execute("CREATE INDEX IF NOT EXISTS idx_daily_msgs_date ON daily_msgs(date)")
                db.execute("CREATE INDEX IF NOT EXISTS idx_daily_msgs_user_date ON daily_msgs(user_id, date)")
                db.execute("CREATE INDEX IF NOT EXISTS idx_users_telegram_id ON users(telegram_id)")
                db.execute("CREATE INDEX IF NOT EXISTS idx_transactions_user_date ON transactions(user_id, created_at)")
                log.info("✅ All critical indexes created successfully")
            except Exception as e:
...
def get_active_platform_account() -> dict | None:
    """Get the currently active platform account."""
    with get_db() as db:
        is_postgres = DATABASE_URL is not None
        if is_postgres:
            row = db.execute(
                "SELECT * FROM platform_accounts WHERE is_active=1 LIMIT 1"
            ).fetchone()
        else:
            row = db.execute(
                "SELECT * FROM platform_accounts WHERE is_active=1 LIMIT 1"
...
def set_active_platform_account(account_id: int) -> bool:
    ...
    try:
        with get_db() as db:
            ...
            if is_postgres:
                db.execute("UPDATE platform_accounts SET is_active=0")
                db.execute("UPDATE platform_accounts SET is_active=1, updated_at=CURRENT_TIMESTAMP WHERE id=%s", (account_id,))
            else:
                db.execute("UPDATE platform_accounts SET is_active=0")
                db.execute("UPDATE platform_accounts SET is_active=1, updated_at=datetime('now') WHERE id=?", (account_id,))
            return True
    except Exception as e:
        log.error(f"Error setting active account: {e}")
        return False


def get_all_platform_accounts() -> list:
    """Get all platform accounts with proper datetime handling."""
    with get_db() as db:
        is_postgres = DATABASE_URL is not None
        if is_postgres:
            rows = db.execute(
                "SELECT * FROM platform_accounts ORDER BY is_active DESC, username"
            ).fetchall()
        else:
            rows = db.execute(
                "SELECT * FROM platform_accounts ORDER BY is_active DESC, username"
        ...
        for row in rows:
            acc = dict(row)
            # Handle datetime objects for safe display
            for key in ['created_at', 'updated_at']:
                if acc.get(key) and hasattr(acc[key], 'strftime'):
                    acc[key] = acc[key].strftime("%Y-%m-%d %H:%M:%S")
            result.append(acc)
        return result


def update_account_numbers_count(username: str, hourly_count: int = None, total_count: int = None):
    """Update the number counts for a platform account."""
    try:
        with get_db() as db:
            is_postgres = DATABASE_URL is not None
            updates = []
            ...
            if total_count is not None:
                updates.append("numbers_count=%s" if is_postgres else "numbers_count=?")
                params.append(total_count)
            if hourly_count is not None:
                updates.append("hourly_numbers_count=%s" if is_postgres else "hourly_numbers_count=?")
                params.append(hourly_count)
            
            if updates:
                updates.append("updated_at=CURRENT_TIMESTAMP" if is_postgres else "updated_at=datetime('now')")
                if is_postgres:
                    db.execute(
                        f"UPDATE platform_accounts SET {', '.join(updates)} WHERE username=%s",
                        params + [username]
                    )
                else:
                    db.execute(
                        f"UPDATE platform_accounts SET {', '.join(updates)} WHERE username=?",
                        params + [username]
                    )
    except Exception as e:
...
def get_next_available_account() -> dict | None:
    """
    Get the next available platform account (the one with the fewest hourly numbers).
    ...
    with get_db() as db:
        ...
        if is_postgres:
            row = db.execute("""
                ...
                ORDER BY hourly_numbers_count ASC, numbers_count ASC
                LIMIT 1
            """).fetchone()
        else:
            row = db.execute("""
...
def should_switch_account(username: str, threshold: int = 110) -> bool:
    """
    Check if an account has exceeded the number threshold.
    Returns True if the account's hourly numbers >= threshold.
    """
    with get_db() as db:
        is_postgres = DATABASE_URL is not None
        if is_postgres:
            row = db.execute(
                "SELECT hourly_numbers_count FROM platform_accounts WHERE username=%s",
                (username,)
            ).fetchone()
        else:
            row = db.execute(
                "SELECT hourly_numbers_count FROM platform_accounts WHERE username=?",
                (username,)
            ).fetchone()
        
        if row:
            return row["hourly_numbers_count"] >= threshold
    return False
...
# ── api_ functions for Manual Mode ─────────────────────────────────────────
# All routed to WSJOBS_BASE_URL (admin.simpletasks88.com) using _wsjobs_* helpers.
# Sign patterns from HAR log:
#   get_code      : md5(md5("/api/user/get_code")      + userid + username + account)
#   get_phonestatus: md5(md5("/api/user/get_phonestatus")+ userid + username + account)
#   addwsnumber   : md5(md5("/api/user/addwsnumber")   + userid + username + account + str(types))
#   sendmsg       : md5(md5("/api/user/sendmsg")       + userid + username + str(wsid))
#   get_appinfo   : md5(md5("/api/user/get_appinfo")   + userid + username)

def _m_ps():
    """Get manual-mode session via wsjobs helpers (simpletasks88.com)."""
    return _wsjobs_ps()
...
def api_get_code(account):
    ...
    try:
        r = _retry(lambda: s["http"].get(f"{WSJOBS_BASE_URL}/api/user/get_code",
                   params={"account": account, "signType": "1", "username": uname, "userid": uid, "sign": sign},
                   headers=_wsjobs_hdrs(), timeout=15), f"code:{account}")
        d = r.json()
        return (str(d["data"]), "ok") if d.get("code") == 0 and d.get("data") else (None, d.get("message", ""))
    except Exception as e:
        log.error(f"[api_get_code] {e}"); return None, 'Service unavailable'

def api_phonestatus(account):
    s = _m_ps(); uid, uname = str(s["userid"]), s["username"]
    sign = _wsjobs_md5(_wsjobs_md5("/api/user/get_phonestatus") + uid + uname + account)
    try:
        r = _retry(lambda: s["http"].get(f"{WSJOBS_BASE_URL}/api/user/get_phonestatus",
                   params={"account": account, "signType": "0", "username": uname, "userid": uid, "sign": sign},
                   headers=_wsjobs_hdrs(), timeout=10), f"status:{account}")
        d = r.json()
        return int(d["data"]) if d.get("code") == 0 else None
    except Exception as e:
        log.error(f"[api_phonestatus] {e}"); return None

def api_addwsnumber(account, types=1):
    s = _m_ps(); uid, uname = str(s["userid"]), s["username"]
    sign = _wsjobs_md5(_wsjobs_md5("/api/user/addwsnumber") + uid + uname + account + str(types))
    try:
        r = _retry(lambda: s["http"].post(f"{WSJOBS_BASE_URL}/api/user/addwsnumber",
                   json={"account": account, "types": types, "username": uname, "userid": int(uid), "sign": sign},
                   headers=_wsjobs_hdrs(), timeout=15), f"addws:{account}")
...
def api_sendmsg(phone, wsid):
    ...
    try:
        r = _retry(lambda: s["http"].post(f"{WSJOBS_BASE_URL}/api/user/sendmsg",
                   ...
                   headers=_wsjobs_hdrs(), timeout=15), f"send:{phone}")
        d = r.json()
        ok = d.get("code") == 0
        if not ok:
...
def api_get_wsid(account):
    ...
    while True:
        ...
        try:
            r = _retry(lambda: s["http"].get(f"{WSJOBS_BASE_URL}/api/user/get_appinfo",
                       params={"page": page, "pagesize": 50, "username": uname, "userid": uid, "sign": sign},
                       headers=_wsjobs_hdrs(), timeout=15), f"wsid:{account}")
            d = r.json()
            if d.get("code") != 0: break
            chunk = d["data"]["list"]; total = d["data"]["count"]
            for item in chunk:
            ...
            log.error(f"[api_get_wsid] {e}"); break
    return None
...
def api_appinfo(page=1, pagesize=50):
    ...
    try:
        r = _retry(lambda: s["http"].get(f"{WSJOBS_BASE_URL}/api/user/get_appinfo",
                   ...
                   headers=_wsjobs_hdrs(), timeout=15), "appinfo")
        d = r.json()
        return (d["data"]["list"], d["data"]["count"]) if d.get("code") == 0 else ([], 0)
    except Exception as e:
        log.error(f"[api_appinfo] {e}"); return [], 0
...
def _wacash_pair_bg(user_id: int, account: str):
    """Wacash mode pairing — country flag, copy button, clean messages."""
    with get_db() as db:
        is_postgres = DATABASE_URL is not None
        if is_postgres:
    ...
    if existing_ws_id:
        ...
        asyncio.run_coroutine_threadsafe(
            send_telegram(telegram_id, f"✅ {country_flag} `{account}` already online and ready!", parse_mode="Markdown"),
            _bot_loop
        )
        return
    ...
    cancel_markup = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data=f"delnum_{account}")]])
    ...
    if not pair_code:
        ...
        _edit(f"❌ {country_flag} Could not get pairing code for `{account}`")
        with _wacash_pairs_lock:
            _wacash_pairs.pop(account, None)
        return
    ...
    _edit(f"🔐 {country_flag} Pairing Code sent successfully\n`{account}`.", code_markup)
    ...
    while time.time() < deadline:
        ...
        if time.time() - last_update > 30:
            ...
            _edit(f"⏳ {country_flag} Waiting for `{account}` to come online… _{remaining}m left_", code_markup)
            last_update = time.time()
    ...
    if not ws_id:
        ...
        _edit(f"⏰ {country_flag} Timeout — `{account}` didn't come online within 5 minutes")
        with _wacash_pairs_lock:
            _wacash_pairs.pop(account, None)
        return
...
def _pair_bg(user_id: int, account: str):
    ...
    with pairs_lock:
        if account in active_pairs:
            active_pairs[account]["pair_code"] = pair_code
    with get_db() as db:
        is_postgres = DATABASE_URL is not None
        if is_postgres:
    ...
    def get_poll_interval(secs):
        if secs < 300: return 3
        elif secs < 1800: return 10
        return 30
    ...
    if wsid:
        ...
        with get_db() as db:
            is_postgres = DATABASE_URL is not None
            if is_postgres:
                db.execute("UPDATE numbers SET status='online',wsid=%s,pair_code=NULL WHERE user_id=%s AND account=%s", (wsid, user_id, account))
                db.execute("DELETE FROM daily_msgs WHERE user_id=%s AND date=%s", (user_id, today))
            else:
                db.execute("UPDATE numbers SET status='online',wsid=?,pair_code=NULL WHERE user_id=? AND account=?", (wsid, user_id, account))
        ...
        with get_db() as db:
            is_postgres = DATABASE_URL is not None
            if is_postgres:
                db.execute("UPDATE numbers SET status='error' WHERE user_id=%s AND account=%s", (user_id, account))
            else:
...
def get_country_flag(number_or_code: str) -> str:
    """Detect country flag emoji from phone number or country code."""
    flags = {
        # Africa
        "20": "🇪🇬",   # Egypt
        "212": "🇲🇦",  # Morocco
    ...
    clean = str(number_or_code).lstrip("+").replace(" ", "")
...
def _pair_hourly_bg(user_id: int, account: str,
    ...
    - Country flag detection from number prefix
    - Copy-to-clipboard inline button for pairing code
    - Next Pairing Code inline button for next variant
    - Clean minimal messages (edit-in-place)
    - Online notification when number connects
    ...
    # ── Auto-switch account if needed ────────────────────────────────────────
    current_account = get_active_platform_account()
    if current_account and should_switch_account(current_account["username"], 110):
        ...
        if next_account and next_account["username"] != current_account["username"]:
            ...
            if platform_login():
                set_active_platform_account(next_account["id"])
                asyncio.run_coroutine_threadsafe(
                    send_telegram(ADMIN_TELEGRAM_ID,
                        f"🔄 Switched to `{next_account['username']}` (110+ limit reached)",
                        parse_mode="Markdown"), _bot_loop)
            else:
                asyncio.run_coroutine_threadsafe(
                    send_telegram(ADMIN_TELEGRAM_ID,
                        f"⚠️ Switch to `{next_account['username']}` failed — staying on current",
                        parse_mode="Markdown"), _bot_loop)

    # ── Ensure WSJOBS session ─────────────────────────────────────────────────
    with wsjobs_lock:
    ...
    if variants_enabled:
        variants = _build_variants(original, account, country_prefix, local_part)[:4 - _variant_count]
    else:
        variants = [account]

    # ── Insert DB rows for each variant ──────────────────────────────────────
    for variant in variants:
        with get_db() as db:
            is_postgres = DATABASE_URL is not None
            if is_postgres:
                existing = db.execute("SELECT id FROM numbers WHERE user_id=%s AND account=%s", (user_id, variant)).fetchone()
            else:
                existing = db.execute("SELECT id FROM numbers WHERE user_id=? AND account=?", (user_id, variant)).fetchone()
            if not existing:
                if is_postgres:
                    db.execute("INSERT INTO numbers(user_id, account, status, hourly_status) VALUES(%s,%s,'pairing','pending')", (user_id, variant))
                else:
                    db.execute("INSERT INTO numbers(user_id, account, status, hourly_status) VALUES(?,?,'pairing','pending')", (user_id, variant))

    # ── Country flag ──────────────────────────────────────────────────────────
    country_flag = get_country_flag(country_prefix) if country_prefix else get_country_flag(account)

    # ── Initial message (only sent once) ─────────────────────────────────────
    initial_msg = _message
    cancel_markup = InlineKeyboardMarkup([[
        InlineKeyboardButton("❌ Cancel", callback_data=f"cancel_pair_{account}")
    ]])

    if initial_msg is None:
        future = asyncio.run_coroutine_threadsafe(
            send_telegram(telegram_id,
                f"🔗 {country_flag} Linking `{account}`",
                parse_mode="Markdown"),
            _bot_loop
        )
        try:
    ...
    for attempt_idx, variant in enumerate(variants):
        slot = _variant_count + attempt_idx + 1

        # Update message: getting code
        asyncio.run_coroutine_threadsafe(
            edit_telegram_message(
                initial_msg,
                f"🔄 {country_flag} `{variant}` — Getting code... ({slot}/4)",
                parse_mode="Markdown",
        ...
        with get_db() as db:
            is_postgres = DATABASE_URL is not None
            if is_postgres:
                db.execute("UPDATE numbers SET status='pairing', hourly_status='pending' WHERE user_id=%s AND account=%s", (user_id, variant))
            else:
                db.execute("UPDATE numbers SET status='pairing', hourly_status='pending' WHERE user_id=? AND account=?", (user_id, variant))
        ...
        if code:
            ...
            log.info(f"[HourlyPair] ✅ Code for '{variant}': {code}")
            break

        log.warning(f"[HourlyPair] No code for {variant}: {err}")
        if attempt_idx == len(variants) - 1:
            for v in variants:
                if v != variant:
                    with get_db() as db:
                        is_postgres = DATABASE_URL is not None
    ...
    # ── Build pairing code message with inline buttons ────────────────────────
    slot = _variant_count + variants.index(used_account) + 1

    # Find next variant for "Next Pairing Code" button
    next_variant_for_btn = None
    if variants_enabled:
        curr_idx = variants.index(used_account) if used_account in variants else 0
        if curr_idx + 1 < len(variants):
            next_variant_for_btn = variants[curr_idx + 1]

    # Build button rows
    #   Row 1: Copy Pairing Code (uses copy_text — native Telegram clipboard)
    #   Row 2: Next Pairing Code (only if another variant exists)
    #   Row 3: Cancel
    btn_rows = [
        [InlineKeyboardButton("📋 Copy Pairing Code", copy_text=pair_code)],
    ]
    if next_variant_for_btn:
        btn_rows.append([
            InlineKeyboardButton(
                "➡️ Next Pairing Code",
                callback_data=f"linkagain_{original}__{next_variant_for_btn}"
            )
        ])
    btn_rows.append([InlineKeyboardButton("❌ Cancel", callback_data=f"cancel_pair_{used_account}")])

    code_markup = InlineKeyboardMarkup(btn_rows)
    ...
    while time.time() < deadline:
        if time.time() - last_update > 30:
            ...
            asyncio.run_coroutine_threadsafe(
                edit_telegram_message(
                    initial_msg,
                    f"⏳ {country_flag} Waiting for `{used_account}` to come online… _{remaining}m left_",
                    parse_mode="Markdown",
                    reply_markup=code_markup
                ), _bot_loop
            )
    ...
    if not came_online:
        asyncio.run_coroutine_threadsafe(
            edit_telegram_message(
                initial_msg,
                f"⏰ {country_flag} Timeout — `{used_account}` didn't come online",
                parse_mode="Markdown"
            ), _bot_loop
        )
        with get_db() as db:
    ...
    asyncio.run_coroutine_threadsafe(
        edit_telegram_message(
            initial_msg,
            f"✅ {country_flag} `{used_account}` online! +₦{rate}/hr",
            parse_mode="Markdown"
        ), _bot_loop
    )
    log.info(f"[HourlyPair] {used_account} ready (original={original})")

    # ── Auto-start next variant ───────────────────────────────────────────────
    next_variant_count = _variant_count + 1
    if variants_enabled and next_variant_count < 4:
        curr_idx = variants.index(used_account) if used_account in variants else 0
        if curr_idx + 1 < len(variants):
            next_v = variants[curr_idx + 1]
            log.info(f"[HourlyPair] 🚀 Auto-starting next variant: {next_v}")
            threading.Thread(
                ...
                kwargs={
                    ...
                    "_message": None  # Fresh message for each variant
                },
                daemon=True
            ).start()
            ...
            if event == "TASK_RESULT":
                ...
                if pts > 0:
                    ...
                    if u_ref and u_ref["referred_by"]:
                        ...
                        _credit(db, u_ref["referred_by"], ref_bonus,
                                f"Ref bonus from uid={user_id} auto", "earn")
                ...
                if pts > 0:
                    ...
                    if u_ref and u_ref["referred_by"]:
                        ...
                        # ✅ FIXED: Changed type from "referral" to "earn" for consistency
                        _credit(db, u_ref["referred_by"], ref_bonus,
                                f"Ref bonus from uid={user_id} auto batch", "earn")
                    bal = db.execute("SELECT balance FROM users WHERE id=?", (user_id,)).fetchone()
                    bal_pts = int(bal["balance"]) if bal else 0
                    ngn = pts_to_ngn(pts)
                    await send_telegram(
...
admin_panel_buttons = [
    ...
    [InlineKeyboardButton("🔐 Manage Accounts", callback_data="admin_accounts", style="primary")],
    [InlineKeyboardButton("📋 List All Accounts", callback_data="admin_list_accounts", style="primary")],
    [InlineKeyboardButton("📊 Account Stats", callback_data="admin_account_stats", style="primary")],
    [InlineKeyboardButton("🔄 Sync Account Counts", callback_data="admin_sync_counts", style="primary")],
    # ================================================
...
async def send_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "wacash":
        ...
        msg = await update.message.reply_text("🔄 **Sending messages simultaneously...**\n⏳ Please wait...", parse_mode="Markdown")
        ...
        async def background_send():
            try:
                ...
                def send_once(acct, ws_id):
                    """Send ONE message per number."""
                    ok, err = wacash_send_msg(ws_id)
                    with per_num_lock:
                        per_num_results[acct] = {"success": 1 if ok else 0, "error": err if not ok else None}
                    return ok

                loop = asyncio.get_running_loop()
                
                # Launch ALL requests simultaneously using ThreadPoolExecutor
                with concurrent.futures.ThreadPoolExecutor(max_workers=len(rows)) as executor:
                    futures = []
                    for r in rows:
                        # Submit all tasks at once - they start immediately
                        future = loop.run_in_executor(executor, send_once, r["account"], r["ws_id"])
                ...
                if total_sent > 0:
                    with get_db() as db:
                        _credit(db, uid, total_earned_pts, f"TaskGo: sent {total_sent} messages")
    ...
    msg = await update.message.reply_text(
        f"🔄 **Sending simultaneously to {num_count} number(s)...**\n"
        f"⏳ Please wait...", 
        parse_mode="Markdown"
    )
    ppm = int(get_setting("points_per_msg", "200"))
    ...
    def send_once(acct, wsid):
        """Send ONE message per number - this runs in a thread."""
        try:
            ok, err = api_sendmsg(acct, wsid)
            with results_lock:
                results[acct] = {"success": ok, "error": err if not ok else None}
            return ok
        except Exception as e:
            with results_lock:
                results[acct] = {"success": False, "error": str(e)}
            return False
    
    async def background_send():
        loop = asyncio.get_event_loop()
        
        # CRITICAL: Use max_workers = number of numbers
        # This ensures ALL requests fire at the same time
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_count) as executor:
            # Submit ALL tasks at once - they all start immediately
            futures = []
            for r in rows:
                future = loop.run_in_executor(executor, send_once, r["account"], r["wsid"])
                futures.append(future)
        ...
        if total_sent > 0:
            with get_db() as db:
                _credit(db, uid, total_earned_pts, f"Manual send-all ({total_sent} messages)")
                _increment_daily_msgs(db, uid, total_sent)
                
                # Update msgs_sent for each successful number
                for r in rows:
                    if results.get(r["account"], {}).get("success", False):
                        db.execute("UPDATE numbers SET msgs_sent=COALESCE(msgs_sent,0)+1 WHERE user_id=? AND account=?", (uid, r["account"]))
...
async def check_in(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as db:
        ...
        # ✅ FIXED: Credit points directly (was: ngn_amt)
        db.execute("INSERT INTO check_ins(user_id,date,points_awarded,streak) VALUES(?,?,?,?)", (uid, today, pts, streak))
        _credit(db, uid, pts, f"Daily check-in bonus (Day {streak})", "checkin")
        
        # Calculate naira value only for display
        ngn_val = pts_to_ngn(pts)
        await update.message.reply_text(f"🎉 Check-in successful! Day {streak} streak.\nYou earned {pts} points! (≈ ₦{ngn_val:.2f})")
        await send_telegram(telegram_id, f"🎉 Daily check-in bonus! +{pts} pts (≈ ₦{ngn_val:.2f}). Keep it up!")
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_stats":
        ...
        # Test login against simpletasks88.com (WSJOBS_BASE_URL) using the active account
        active_acc = get_active_platform_account()
        test_user = active_acc["username"] if active_acc else PLATFORM_USER
        await query.edit_message_text(
            f"🧪 Testing manual mode login (simpletasks88.com)...\n"
            f"👤 Username: `{test_user}`",
            parse_mode="Markdown"
        )
        loop = asyncio.get_event_loop()
        ok = await loop.run_in_executor(None, wsjobs_login)
        if ok:
            with wsjobs_lock:
                uid_on_platform = wsjobs_session.get("userid", "?")
                logged_user = wsjobs_session.get("username", test_user)
            await query.edit_message_text(
                f"✅ *Login Successful!*\n\n"
                f"👤 Username: `{logged_user}`\n"
                f"🆔 Platform UID: `{uid_on_platform}`\n"
                f"🌐 Platform: `{WSJOBS_BASE_URL}`\n\n"
                f"✅ Manual mode is ready — Add Number and Send All will work correctly.",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[
            ...
            await query.edit_message_text(
                ...
                f"👤 Username: `{test_user}`\n"
                f"🌐 Platform: `{WSJOBS_BASE_URL}`\n\n"
                f"Please add/update credentials using\n"
                f"'🔐 Set Manual Login Creds' or `/add_account` and try again.",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[
    ...
    # ========== NEW: HOURLY VARIANTS TOGGLE ==========
    elif data == "admin_hourly_variants":
        """Toggle hourly variants from admin panel"""
        current = get_hourly_variants_enabled()
        new_state = not current
        toggle_hourly_variants(new_state)
        
        await query.edit_message_text(
            f"✅ *Hourly Variants {'Enabled' if new_state else 'Disabled'}*\n\n"
            f"📱 When enabled, the bot will try up to 4 number formats:\n"
            f"   • Original: `2349157338416`\n"
            f"   • +0: `23409157338416`\n"
            f"   • +00: `234009157338416`\n"
            f"   • +000: `2340009157338416`\n"
            f"   • +0000: `23400009157338416`\n\n"
    ...
    elif data == "admin_clear_offline":
        """Show clear offline menu"""
        await query.edit_message_text(
            "📝 *Clear Offline Numbers*\n\n"
            "Send: `/clear_offline <user_id>` to clear all offline numbers for a user.\n"
            "Or: `/clear_offline <user_id> <account>` to clear a specific number.\n\n"
            "⚠️ This marks offline numbers as paid.\n"
            "✅ Use this after paying users for their offline hours.\n\n"
            "Use `/admin_users` to find user IDs.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("◀️ Back to Admin Panel", callback_data="admin_back")
            ]])
    ...
    # ========== ACCOUNT MANAGEMENT HANDLERS ==========
        ...
        if not accounts:
            await query.edit_message_text(
                ...
                "Use `/add_account <username> <password>` to add your first account.\n\n"
                "💡 The bot will auto-switch when an account exceeds 110 numbers.",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
        ...
        for acc in accounts:
            ...
            text += f"{active} *{acc['username']}*\n"
            if acc["label"]:
                text += f"   └ 🏷️ {acc['label']}\n"
            text += f"   └ 📞 {acc['numbers_count']} total\n"
            text += f"   └ ⚡ {acc['hourly_numbers_count']} hourly\n"
            text += "\n"
        
        text += "━━━━━━━━━━━━━━━━━━━━\n"
        text += "⚡ *Auto-switch at 110 numbers*"
        
        await query.edit_message_text(
            text,
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📋 List All Accounts", callback_data="admin_list_accounts")],
                [InlineKeyboardButton("➕ Add Account", callback_data="admin_add_account")],
                [InlineKeyboardButton("🔄 Switch Account", callback_data="admin_switch_account")],
                [InlineKeyboardButton("🔄 Sync Counts", callback_data="admin_sync_counts")],
                [InlineKeyboardButton("📊 Account Stats", callback_data="admin_account_stats")],
                [InlineKeyboardButton("🔧 Hourly Variants", callback_data="admin_hourly_variants")],
                [InlineKeyboardButton("🗑 Clear Offline", callback_data="admin_clear_offline")],
                [InlineKeyboardButton("◀️ Back", callback_data="admin_back")]
            ])
        )

    elif data == "admin_list_accounts":
        """List all platform accounts with detailed info."""
        accounts = get_all_platform_accounts()
        ...
        if not accounts:
            await query.edit_message_text(
                ...
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("◀️ Back", callback_data="admin_accounts")
                ]])
            )
            return
        
        text = "🔐 *Platform Accounts - Full List*\n"
        text += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        ...
        for i, acc in enumerate(accounts, 1):
            ...
            text += f"{active_icon} *{i}. {acc['username']}*\n"
            
            if acc["label"]:
                text += f"   └ 🏷️ Label: {acc['label']}\n"
            
            num_count = acc["numbers_count"] or 0
            hourly_count = acc["hourly_numbers_count"] or 0
        ...
        text += f"📞 Total numbers: {total_numbers}\n"
        text += f"⚡ Hourly numbers: {total_hourly}\n\n"
        
        # Show which account is next in line for auto-switch
        next_account = get_next_available_account()
        if next_account:
            text += f"🔄 *Next account for auto-switch:*\n"
            text += f"   `{next_account['username']}` ({next_account['hourly_numbers_count']} numbers)\n\n"
        
        text += "💡 *Auto-switch triggers at 110 numbers per account*"
        
        await query.edit_message_text(
            text,
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔄 Refresh", callback_data="admin_list_accounts")],
                [InlineKeyboardButton("◀️ Back to Accounts", callback_data="admin_accounts")]
            ])
        )
    ...
    elif data == "admin_switch_account":
        accounts = get_all_platform_accounts()
        if not accounts:
            await query.edit_message_text("No accounts available to switch to.", reply_markup=admin_panel_markup)
            return
        ...
        for acc in accounts:
            if acc["is_active"]:
                buttons.append([InlineKeyboardButton(f"⭐ {acc['username']} (Active)", callback_data="switch_skip")])
            else:
                buttons.append([InlineKeyboardButton(f"🔄 {acc['username']}", callback_data=f"switch_{acc['username']}")])
        buttons.append([InlineKeyboardButton("◀️ Back", callback_data="admin_accounts")])
        ...
        if username == "skip":
            await query.answer("This account is already active.")
            return
    ...
    elif data == "admin_sync_counts":
        await query.edit_message_text("🔄 Syncing account counts...", parse_mode="Markdown")
        
        # Run sync
        accounts = get_all_platform_accounts()
        if not accounts:
            await query.edit_message_text("No accounts to sync.", reply_markup=admin_panel_markup)
            return
        ...
        for acc in accounts:
            ...
            with get_db() as db:
                ...
                if is_postgres:
                    total = db.execute(
                        "SELECT COUNT(*) as c FROM numbers WHERE account LIKE %s",
                        (username + '%',)
                    ).fetchone()["c"]
                    
                    hourly = db.execute(
                        "SELECT COUNT(*) as c FROM numbers n "
                        "JOIN users u ON n.user_id = u.id "
                        "WHERE n.account LIKE %s AND u.earning_mode = 'hourly'",
                        (username + '%',)
                    ).fetchone()["c"]
                else:
                    total = db.execute(
                        "SELECT COUNT(*) as c FROM numbers WHERE account LIKE ?",
                        (username + '%',)
                    ).fetchone()["c"]
                    ...
                    hourly = db.execute(
                        "SELECT COUNT(*) as c FROM numbers n "
                        "JOIN users u ON n.user_id = u.id "
                        "WHERE n.account LIKE ? AND u.earning_mode = 'hourly'",
                        (username + '%',)
                    ).fetchone()["c"]
                
                update_account_numbers_count(username, hourly, total)
                synced_count += 1
                total_numbers += total
                total_hourly += hourly
        ...
        await query.edit_message_text(
            ...
            f"⚡ Hourly numbers: {total_hourly}\n"
            f"👥 Accounts: {synced_count}",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("◀️ Back to Accounts", callback_data="admin_accounts")
            ]])
        ...
        text = (
            f"📊 *Account & Hourly Stats*\n\n"
            f"👥 Platform Accounts: {total_accounts}\n"
            f"⭐ Active: {active_account['username'] if active_account else 'None'}\n\n"
            f"💰 Total hourly paid: {int(total_points):,} pts (≈ ₦{total_ngn:.2f})\n"
            f"👤 Users in hourly: {hourly_users}\n"
            f"📞 Online numbers: {online_numbers}/{total_hourly_numbers}\n"
            f"⚡ Rate: ₦{rate}/hour\n\n"
            f"⚡ *Auto-switch at 110 numbers*"
        )
...
async def add_account_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Add a new platform account (admin only)."""
    if update.effective_user.id != ADMIN_TELEGRAM_ID:
        await update.message.reply_text("Unauthorized.")
        return
...
async def list_accounts_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """List all platform accounts with their counts."""
    if update.effective_user.id != ADMIN_TELEGRAM_ID:
        await update.message.reply_text("Unauthorized.")
        return
    ...
    for acc in accounts:
        active_icon = "⭐" if acc["is_active"] else "  "
        text += f"{active_icon} *{acc['username']}*\n"
        if acc["label"]:
            text += f"   └ 🏷️ {acc['label']}\n"
        text += f"   └ 📞 Total: {acc['numbers_count']} numbers\n"
        text += f"   └ ⚡ Hourly: {acc['hourly_numbers_count']} numbers\n"
        text += f"   └ 📅 Added: {acc['created_at'][:16] if acc['created_at'] else 'Unknown'}\n"
        text += "\n"
    
    text += "━━━━━━━━━━━━━━━━━━━━\n"
    text += "💡 *Auto-Switch Rule:*\n"
    text += "When an account reaches **110** hourly numbers,\n"
    text += "the system auto-switches to the next account\n"
    text += "with the fewest numbers.\n\n"
    text += "📝 *Commands:*\n"
    text += "`/add_account <user> <pass> [label]`\n"
    text += "`/switch_account <username>` - manually switch\n"
    text += "`/remove_account <username>` - delete account\n"
    text += "`/sync_accounts` - update number counts"
...
async def remove_account_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) != 1:
        await update.message.reply_text("Usage: `/remove_account <username>`", parse_mode="Markdown")
        return
    ...
    if active and active["username"] == username:
        await update.message.reply_text(
            f"⚠️ Cannot remove active account `{username}`.\n"
            f"Use `/switch_account <other>` first.",
            parse_mode="Markdown"
        )
        return
    ...
    with get_db() as db:
        ...
        # Delete the account
        if is_postgres:
            db.execute("DELETE FROM platform_accounts WHERE username=%s", (username,))
        else:
            db.execute("DELETE FROM platform_accounts WHERE username=?", (username,))
    
    await update.message.reply_text(f"✅ Removed account `{username}`", parse_mode="Markdown")


async def sync_accounts_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Manually sync account number counts."""
    if update.effective_user.id != ADMIN_TELEGRAM_ID:
        await update.message.reply_text("Unauthorized.")
        return
    
    msg = await update.message.reply_text("🔄 Syncing account number counts...", parse_mode="Markdown")
    
    # Get all accounts
    accounts = get_all_platform_accounts()
    
    if not accounts:
        await msg.edit_text("📭 No accounts found to sync.", parse_mode="Markdown")
        return
    ...
    for acc in accounts:
        ...
        # Count numbers for this account
        with get_db() as db:
            is_postgres = DATABASE_URL is not None
            
            # Count total numbers (all modes)
            if is_postgres:
                total = db.execute(
                    "SELECT COUNT(*) as c FROM numbers WHERE account LIKE %s",
                    (username + '%',)
    ...
    await msg.edit_text(
        f"✅ *Accounts Synced Successfully*\n\n"
        f"📊 Total numbers: {total_numbers}\n"
        f"⚡ Hourly numbers: {total_hourly}\n"
        f"👥 Accounts synced: {synced_count}\n\n"
        f"Use `/list_accounts` to see updated counts.",
        parse_mode="Markdown"
    )  
...
async def toggle_hourly_variants_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Toggle hourly variants on/off (admin only)"""
    if update.effective_user.id != ADMIN_TELEGRAM_ID:
        await update.message.reply_text("❌ Unauthorized.")
        return
    ...
    if len(args) != 1 or args[0].lower() not in ["on", "off"]:
        await update.message.reply_text(
            "📝 *Toggle Hourly Variants*\n\n"
            "Usage: `/toggle_hourly_variants on|off`\n\n"
            "When ON (default): The bot tries up to 4 variants per number in hourly mode.\n"
            "When OFF: The bot only tries the exact number once.\n\n"
            "Current status: `{}`".format(
                "ON" if get_hourly_variants_enabled() else "OFF"
            ),
            parse_mode="Markdown"
        )
        return
...
async def clear_offline_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 1:
        await update.message.reply_text(
            ...
            "`/clear_offline <user_id>` - clear all offline numbers for a user\n"
            "`/clear_offline <user_id> <account>` - clear a specific offline number\n\n"
            "⚠️ This marks offline numbers as paid so they won't appear in admin lists.\n"
            "✅ Use this after paying users for their offline hours.",
            parse_mode="Markdown"
        )
        return
    ...
    with get_db() as db:
        if account:
            ...
            _admin_log(db, get_internal_user_id(ADMIN_TELEGRAM_ID), "clear_offline",
                       f"user_id={uid}", f"Number {account} marked as paid")
            await update.message.reply_text(f"✅ Offline number `{account}` for user {uid} has been marked as cleared/paid.", parse_mode="Markdown")
        else:
            # Count how many will be cleared
            result = db.execute("""
                SELECT COUNT(*) as c, COALESCE(SUM(total_hours_earned), 0) as total_hours 
                FROM numbers 
                WHERE user_id = ?
                AND hourly_status = 'offline'
                AND COALESCE(offline_hours_processed, 0) = 0
            ...
            if count == 0:
                await update.message.reply_text(f"📭 No unpaid offline numbers found for user {uid}.", parse_mode="Markdown")
                return
            ...
            """, (uid,))
            _admin_log(db, get_internal_user_id(ADMIN_TELEGRAM_ID), "clear_offline",
                       f"user_id={uid}", f"Cleared {count} offline numbers with {total_hours} hours (₦{total_ngn:.2f})")
            
            await update.message.reply_text(
                f"✅ *Cleared {count} Offline Number(s)*\n\n"
                f"👤 User ID: `{uid}`\n"
                f"⏱ Total hours: `{total_hours} h`\n"
                f"💰 Total value: `₦{total_ngn:.2f}`\n\n"
                f"These numbers have been marked as paid and removed from the admin list.",
                parse_mode="Markdown"
            )
        
# ============ BACKGROUND ACCOUNT COUNT SYNC ============

async def account_count_sync():
    """
    Background task: Periodically update platform account number counts.
    This helps the auto-switch logic make informed decisions.
    ...
    while True:
        try:
            ...
            log.info("[AccountSync] Updated account counts for all accounts")
            
        except Exception as e:
            log.error(f"[AccountSync] Error: {e}")
...
async def post_init(app):
    ...
    # ============ START ACCOUNT COUNT SYNC ============
    asyncio.create_task(account_count_sync())
    log.info("[Speed] Account count sync started")
...
# ============ FIX #6: AUDIT BALANCES COMMAND (Admin) ============
async def audit_balances(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin command to audit user balances against transactions (FIXED: Bug #2)"""
    if update.effective_user.id != ADMIN_TELEGRAM_ID:
        await update.message.reply_text("❌ Admin only")
        return
    
    await update.message.reply_text("⏳ Auditing balances... (this may take a minute)")
    
    issues = []
    
    with get_db() as db:
        # Get all users
        users = db.execute("SELECT id, username, balance FROM users ORDER BY id").fetchall()
        ...
        for user in users:
            ...
            result = db.execute("""
                SELECT 
                    COALESCE(SUM(CASE WHEN type IN ('earn', 'referral', 'checkin') THEN amount ELSE 0 END), 0) as earned,
                    COALESCE(SUM(CASE WHEN type IN ('debit', 'withdrawal') THEN amount ELSE 0 END), 0) as debited
                FROM transactions
                WHERE user_id = ?
            """, (uid,)).fetchone()
            
            earned = int(result["earned"]) if result["earned"] else 0
            debited = int(result["debited"]) if result["debited"] else 0
            calculated_balance = earned - debited
            
            # Compare
            if calculated_balance != current_balance:
                issues.append({
                    'user': f"{username} (ID: {uid})",
                    'stored': current_balance,
                    'calculated': calculated_balance,
                    'diff': current_balance - calculated_balance
                })
    
    if not issues:
        await update.message.reply_text("✅ All balances match! No issues found.")
        return
    
    # Report issues
    report = "🚨 *Balance Audit Report*\n\n"
    report += f"Found {len(issues)} discrepancies:\n\n"
    
    for issue in issues[:15]:  # Show first 15
        report += (
            f"👤 {issue['user']}\n"
            f"  Stored: {issue['stored']:,}\n"
            f"  Calculated: {issue['calculated']:,}\n"
            f"  Diff: {issue['diff']:,}\n\n"
        )
...
def main():
    ...
    # ============ NEW: HOURLY VARIANTS TOGGLE COMMAND ============
    application.add_handler(CommandHandler("toggle_hourly_variants", toggle_hourly_variants_command), group=-1)

    # ============ NEW: ACCOUNT MANAGEMENT COMMANDS ============
    application.add_handler(CommandHandler("add_account", add_account_command), group=-1)
    application.add_handler(CommandHandler("list_accounts", list_accounts_command), group=-1)
    ...
    application.add_handler(CommandHandler("test_account", test_account_command), group=-1)
