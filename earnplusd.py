...
def _pair_hourly_bg(user_id: int, account: str,
    ...
    came_online = False
    while time.time() < deadline:
        # Check if this variant is online
...
def _pair_hourly_bg(user_id: int, account: str,
    ...
    # Check if we need to switch (threshold = 110 numbers)
    if current_account and should_switch_account(current_account["username"], 110):
        log.info(f"[HourlyPair] Account {current_account['username']} has >= 110 numbers. Switching...")
        
        # Find next available account with fewer numbers
        next_account = get_next_available_account()
        ...
        if next_account and next_account["username"] != current_account["username"]:
            ...
            if platform_login():
                log.info(f"[HourlyPair] Successfully switched to {next_account['username']}")
                # Update active account in DB
                set_active_platform_account(next_account["id"])
                
                # Notify admin about switch
                asyncio.run_coroutine_threadsafe(
                    send_telegram(
                        ADMIN_TELEGRAM_ID,
                        f"🔄 *Account Auto-Switch*\n\n"
    ...
    # ============ CONTINUE WITH EXISTING PAIRING CODE ============
    # Everything below here is your existing _pair_hourly_bg code
    # Keep ALL of it unchanged from here!
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ========== END ACCOUNT MANAGEMENT HANDLERS ==========
